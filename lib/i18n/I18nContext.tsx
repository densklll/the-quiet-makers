...
export function I18nProvider({ initialLocale = "en", children }: { initialLocale?: Locale; children: React.ReactNode }) {
  ...
  const [messages, setMessages] = useState<Messages>(initialLocale === "ru" ? ruDict : enDict);
  ...
  useEffect(() => {
    // Синхронно переключаем сообщения, чтобы избежать мерцания ключей
    setMessages(locale === "ru" ? ruDict : enDict);
